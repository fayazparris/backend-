...
This section explains how to export and use the Mermaid diagrams in technical documents (Word, PDF, PowerPoint, etc.).

### Method 1: Export from Mermaid Live (Recommended)

1. **Copy the Mermaid code** from any diagram section in this README (the code between ` ```mermaid ` and ` ``` `).

2. **Go to Mermaid Live Editor**: Open `https://mermaid.live` in your browser.

3. **Paste the code** into the editor on the left side.

4. **Export the diagram**:
   - Click **"Actions"** button (top right)
   - Select **"Download PNG"** (for raster images) or **"Download SVG"** (for vector images, recommended for documents)
   - Save the file with a descriptive name (e.g., `system-context-diagram.png`)

5. **Insert into your document**:
   - **Microsoft Word**: Insert → Pictures → This Device → Select the exported image
   - **Google Docs**: Insert → Image → Upload from computer
   - **LaTeX**: Use `\includegraphics{diagram.png}` in your document
   - **Markdown**: Use `![Diagram Description](path/to/diagram.png)`

### Method 2: Screenshot from GitHub/GitLab

If your README is hosted on GitHub or GitLab:
...
4. Insert the screenshot into your document.

### Method 3: Using Mermaid CLI (For Batch Export)

If you need to export multiple diagrams:
...
### Best Practices for Technical Documents

1. **Figure Numbering**: Number your diagrams sequentially (Figure 1, Figure 2, etc.)

2. **Captions**: Always include captions below diagrams:
   ```
   Figure 1: System Context Diagram showing the relationship between users, 
   frontend, backend API, and Oracle database.
   ```

3. **References in Text**: Reference diagrams in your text:
   ```
   As shown in Figure 1, the system consists of three main components...
   ```

4. **Image Quality**:
   - Use **SVG** format for scalability (best for documents)
   - Use **PNG** with high DPI (300 DPI minimum) for print documents
   - Ensure diagrams are readable at the size they'll appear in the document

5. **File Organization**: Create a `diagrams/` folder in your document project:
   ```
   technical-document/
   ├── diagrams/
   │   ├── 01-system-context.png
   │   ├── 02-container-diagram.png
   │   ├── 03-component-diagram.png
   │   └── ...
   ├── chapters/
   └── document.docx
   ```

### Quick Reference: All Diagram Codes

For easy access, here are all diagram codes ready to copy:
...
*(Note: See the full README for all other diagram codes - Authentication Flow, Middleware Pipeline, Deployment Architecture, Data Flow, and Domain Model ER Diagram)*

### Example Document Structure

When creating a technical document, organize diagrams like this:
...
3. Architecture Details
   ...
   - Figure 4: Middleware Pipeline Diagram
4. Data Model
   - Figure 5: Domain Model ER Diagram
5. Authentication & Security
   - Figure 6: Authentication Flow Diagram
6. Deployment
   - Figure 7: Deployment Architecture Diagram
7. System Flows
   - Figure 8: Request/Response Flow Diagram
   - Figure 9: Data Flow Diagram
```

---

This README is intended as a **technical reference** for developers working on or integrating with the PHR backend. For any missing details, refer directly to the code in the corresponding folders (`Controllers`, `Services`, `Repositories`, `Models`, `Data`, and `Middleware`).

# capstone-backend-phr
